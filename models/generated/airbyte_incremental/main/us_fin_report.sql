...
from (
         select  DATE_FORMAT(CONVERT_TZ(`posted-date`, '+00:00', '-8:00') , '%Y-%m-01')       as month,
                 CONVERT_TZ(TIMESTAMP(DATE_FORMAT(`posted-date`, '%Y-%m-%d %H:%i:%s.%f')), '+00:00', '-8:00')   as date_time,
         ...
         and `posted-date`!=''
         and  CONVERT_TZ(`posted-date`, '+00:00', '-8:00')>= DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 3 MONTH), '%Y-%m-01')
